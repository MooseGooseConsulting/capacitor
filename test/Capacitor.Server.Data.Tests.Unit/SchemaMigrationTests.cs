...
public class SchemaMigrationTests {
    ...
    [Test]
    public async Task postgres_analytics_views_use_create_or_replace() {
        var viewsSql = await SqliteDatabaseInitializer.GetEmbeddedMigrationAsync("002_analytics_views.postgres.sql");
        await Assert.That(viewsSql.Contains("CREATE OR REPLACE VIEW", StringComparison.Ordinal)).IsTrue();
        await Assert.That(viewsSql.Contains("CREATE VIEW IF NOT EXISTS", StringComparison.OrdinalIgnoreCase)).IsFalse();
    }
