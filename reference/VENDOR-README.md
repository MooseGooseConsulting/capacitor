...
A Copilot reviewer can read your working tree — uncommitted and untracked changes included — from a daemon-owned snapshot. Because that requires giving it real read tools, the containment boundary moves below kcap into an OS sandbox, and the capability is only advertised where all three of these hold:
...
> Kiro intermittently raises a permission prompt for a tool that *is* in its own trust list (an kcap
...
IDE installed never offers kcap at all.
...
rather it asked first, restrict kcap with [`kcap daemon consent`](#launch-consent-kcap-daemon-consent).
...
connected daemon advertises kcap and cannot say why.
...
asked for), the daemon transparently resumes the session where kcap supports it: it relaunches
