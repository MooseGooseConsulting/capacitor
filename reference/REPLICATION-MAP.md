...
Public 2026-08-29 snapshot (frozen, not this checkout):
[claude.ai artifact](https://claude.ai/code/artifact/4a6ed393-4991-4d62-9835-4e578a67cce8).

This file is the live map. Inspected at `origin/main` **`5ae4a671`**
(`feat(server): persist Sessions list and detail from captured events`).
Every claim traces to that tree, `reference/SURFACE.md`, or
`reference/WAVES.md`. Percentages in the snapshot were estimates from
that inspection, not measurements; this rewrite does not invent a
...
The snapshot closed the gap between “what kcap has” and “what we have”
for 2026-08-29. This file is that status column, pinned to
