-- ============================================================================
-- 002_analytics_views.postgres.sql: Analytics Views for Capacitor Server (PostgreSQL)
-- PostgreSQL's CREATE VIEW grammar has no conditional-create clause (only tables,
-- indexes and materialized views get one), so this form uses CREATE OR REPLACE VIEW.
-- Keep in sync with the SQLite form.
-- ============================================================================

CREATE OR REPLACE VIEW v_an_sessions AS
SELECT
    s.repo_hash,
    s.session_id,
    s.model,
...
CREATE OR REPLACE VIEW v_an_token_usage_by_model AS
SELECT
    s.repo_hash,
    e.vendor,
    e.model,
...
CREATE OR REPLACE VIEW v_an_tool_usage AS
SELECT
    s.repo_hash,
    e.vendor,
    e.tool_name,
...
CREATE OR REPLACE VIEW v_an_eval_scores AS
SELECT
    s.repo_hash,
    r.session_id,
    r.eval_run_id,
...
CREATE OR REPLACE VIEW v_an_work_items AS
SELECT
    w.repo_hash,
    w.work_item_id,
    w.title,
