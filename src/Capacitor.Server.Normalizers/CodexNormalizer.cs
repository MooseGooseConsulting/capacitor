...
public sealed class CodexNormalizer : INormalizer {
    ...
    static IReadOnlyList<SessionEventRecord> NormalizeMessage(
        ...
        // Rollout item usage is not an independent billing record: Codex later emits
        // event_msg.token_count.total_token_usage for the full stream. Keep the
        // content event unmetered so that cumulative snapshot is the single source.
    ...
    static IReadOnlyList<SessionEventRecord> NormalizeReasoning(
        ...
        var result = new List<SessionEventRecord>();
        var model = payload.Str("model");
        // See NormalizeMessage: the stream-level token_count snapshot owns usage.
        var usage = default(Usage);
        var usageAttached = false;
    ...
    static IReadOnlyList<SessionEventRecord> NormalizeEventMessage(
        ...
        return [Frame(sessionId, agentId, lineNumber, rawLine, timestamp, "UsageSnapshot",
