...
public class PostgresSessionRepository : ISessionRepository {
    ...
    public async Task<SessionSearchPage> SearchSessionsAsync(SessionSearchQuery query, CancellationToken ct = default) {
        ...
        await using (var count = conn.CreateCommand()) {
            ...
            AddSearchParameters(count, vendor, status, repo, author, text);
        ...
        cmd.CommandText = $@"
            ...
            LIMIT $6 OFFSET $7;";
        AddSearchParameters(cmd, vendor, status, repo, author, text);
    ...
    private static void AddSearchParameters(
        ...
        command.Parameters.Add(new NpgsqlParameter { NpgsqlDbType = NpgsqlDbType.Text, Value = (object?)author ?? DBNull.Value });
        ...
        if (patch?.PreviousSessionId is { Length: > 0 } previousSessionId) {
            await using var link = new NpgsqlCommand(@"
                ...
                WHERE session_id = $2
                  AND (next_session_id IS NULL OR next_session_id = $1);",
                conn, tx);
            link.Parameters.AddWithValue(sessionId);
            link.Parameters.AddWithValue(previousSessionId);
            await link.ExecuteNonQueryAsync(ct);
        }
