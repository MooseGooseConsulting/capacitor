...
public class PostgresSessionRepository : ISessionRepository {
    ...
    public async Task<IReadOnlyList<SessionHeaderRecord>> SearchSessionsAsync(
            string? query,
            string? author,
            string? repo,
            int limit,
        ...
        cmd.CommandText = @"
            SELECT session_id, title, slug, vendor, model, status, visibility, owner_user_id, machine_id, daemon_id,
                   ...
                   previous_session_id, next_session_id, primary_phase, secondary_phase, classification_confidence, classification_source
            FROM sessions
            WHERE ($1::text IS NULL OR title ILIKE $1 OR slug ILIKE $1 OR session_id ILIKE $1)
              AND ($2::text IS NULL OR owner_user_id ILIKE $2)
              AND ($3::text IS NULL
                   OR repo_hash = $3
                   OR (repo_owner || '/' || repo_name) = $3)
            ORDER BY COALESCE(last_event_at, started_at) DESC
            LIMIT $4 OFFSET $5;";
        cmd.Parameters.AddWithValue((object?)(query is null ? null : $"%{query}%") ?? DBNull.Value);
        cmd.Parameters.AddWithValue((object?)(author is null ? null : $"%{author}%") ?? DBNull.Value);
    ...
    static SessionHeaderRecord ReadSession(NpgsqlDataReader reader) => new() {
