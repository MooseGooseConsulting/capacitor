...
current in-repo PostgreSQL implementation, with a focused commit for each
durable unit; it does not mean force-merging or deleting the original branches.

| PR | Original focus | Recovery decision |
| --- | --- | --- |
| #14 | HTTP gateway and API contracts | Reuse route and client-contract research, identifier handling, and useful evaluation context shape. Rewrite persistence-facing routes against the current PostgreSQL interfaces; do not revive the SQLite gateway. |
